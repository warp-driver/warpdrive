...
impl<S: CAStorage + Send + Sync + 'static> WasmEngine<S> {
    ...
    /// This will execute a contract that implements the warpdrive:operator wit interface
    ...
    pub async fn execute_operator_component(
        ...
        #[cfg(feature = "dev")]
        if std::env::var("WARPDRIVE_FORCE_ENGINE_ERROR_XXX").is_ok() {
        ...
        #[cfg(feature = "dev")]
        if std::env::var("WARPDRIVE_FORCE_SLOW_ENGINE_XXX").is_ok() {
        ...
        tracing::info!(
            ...
            "WASM vector execution completed"
        );
...
pub mod tests {
    ...
    async fn validate_execute_config_environment() {
        ...
        std::env::set_var("WARPDRIVE_ENV_TEST", "testing");
        std::env::set_var("WARPDRIVE_ENV_TEST_NOT_ALLOWED", "secret");
        ...
        workflow.component.env_keys = ["WARPDRIVE_ENV_TEST".to_string()].into_iter().collect();
        ...
        let results = engine
            .execute_operator_component(
                ...
                TriggerAction {
                    ...
                    data: TriggerData::new_raw(br#"envvar:WARPDRIVE_ENV_TEST"#),
                },
            )
            .await
            .unwrap();
        ...
        let result = engine
            .execute_operator_component(
                ...
                TriggerAction {
                    ...
                    data: TriggerData::new_raw(br#"envvar:WARPDRIVE_ENV_TEST_NOT_ALLOWED"#),
                },
            )
            .await
            .unwrap_err();
